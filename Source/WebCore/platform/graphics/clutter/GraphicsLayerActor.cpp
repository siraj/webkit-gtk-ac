...
static void graphics_layer_actor_class_init(GraphicsLayerActorClass* klass)
{
    ...
    pspec = g_param_spec_float("translate-x",
                               "Translate X",
                               "Translation value for the X axis",
                                -G_MAXFLOAT,
                                 G_MAXFLOAT,
                                 0.0,
                                 static_cast<GParamFlags>(G_PARAM_READWRITE));
    g_object_class_install_property(objectClass, PROP_TRANSLATE_X, pspec);

    pspec = g_param_spec_float("translate-y",
                               "Translate Y",
                               "Translation value for the Y ayis",
                               -G_MAXFLOAT,
                                G_MAXFLOAT,
                                0.0,
                                static_cast<GParamFlags>(G_PARAM_READWRITE));
    g_object_class_install_property(objectClass, PROP_TRANSLATE_Y, pspec);
}
...
{
    GraphicsLayerActor* graphicsLayer = GRAPHICS_LAYER_ACTOR(actor);
    GraphicsLayerActorPrivate* priv = graphicsLayer->priv;

    CLUTTER_ACTOR_CLASS(graphics_layer_actor_parent_class)->pick(actor, pick);
...
static gboolean graphicsLayerActorDraw(ClutterCairoTexture* texture, cairo_t* cr, GraphicsLayerActor* layer)
{
    ...
    if (priv->surface) {
        int surfaceWidth = cairo_image_surface_get_width(priv->surface.get());
        int surfaceHeight = cairo_image_surface_get_height(priv->surface.get());
    ...
    if (priv->layerType == GraphicsLayerClutter::LayerTypeWebLayer)
        graphicsLayerActorDrawLayerContents(actor, context);
...
void graphicsLayerActorSetAnchorPoint(GraphicsLayerActor* layer, gfloat x, gfloat y, gfloat z)
{
    GraphicsLayerActorPrivate* priv = layer->priv;
...
void graphicsLayerActorGetAnchorPoint(GraphicsLayerActor* layer, gfloat* x, gfloat* y, gfloat* z)
{
    GraphicsLayerActorPrivate* priv = layer->priv;
    if (x)
        *x = priv->anchorX;
...
void graphicsLayerActorSetScrollPosition(GraphicsLayerActor* layer, gfloat x, gfloat y)
{
    GraphicsLayerActorPrivate* priv = layer->priv;
